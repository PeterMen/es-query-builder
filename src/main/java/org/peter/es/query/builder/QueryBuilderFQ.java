...
public class QueryBuilderFQ extends BaseQueryBuilder implements QueryBuilder {
    ...
    public void buildQuery(MyQuery myQuery, QueryParamWrapper esQuery, String paramName, Object paramValue){
        ...
        if(paramValueStr.startsWith(EXCLAMATION)){
            mustNot = true;
            // 去除感叹号
            paramValueStr = paramValueStr.substring(1);
            if(paramValueStr.startsWith(COLON)){
        ...
        // 判断是否nested结构查询
        boolean nested = myQuery.isNested(esName);

        int charAt = paramValueStr.indexOf(COLON);
        if(charAt == -1){
            // 日期格式中，冒号需要转义
            buildNoRule(boolQueryBuilder, esName, mustNot, nested, paramValueStr.replaceAll(COLON_BEHALF, COLON));
        } else {

            // 规则名，规则值
            String ruleName = paramValueStr.substring(0, charAt);
            String ruleVal = paramValueStr.substring(charAt+1).replaceAll(COLON_BEHALF, COLON);
